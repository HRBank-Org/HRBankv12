          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                {/* Task List */}
                {(editingRole.standard_tasks || []).length > 0 && (
                  <div className="mb-3 space-y-2">
                    {(editingRole.standard_tasks || []).map((task, idx) => (
                      <div key={idx} className="flex items-center gap-2 p-2 bg-gray-50 rounded border border-gray-200">
                        <span className="text-gray-400">☐</span>
                        <span className="flex-1 text-sm text-gray-900">{task}</span>
                        <button
                          ...
                          onClick={() => setEditingRole({
                            ...editingRole,
                            standard_tasks: (editingRole.standard_tasks || []).filter((_, i) => i !== idx)
                          })}
                          className="text-red-500 hover:text-red-700 text-xs"
                        >
                          Remove
                  ...
                  <input
                    ...
                    onKeyPress={(e) => {
                      if (e.key === 'Enter' && e.target.value.trim()) {
                        ...
                        setEditingRole({
                          ...editingRole,
                          standard_tasks: [...(editingRole.standard_tasks || []), e.target.value.trim()]
                        });
                        e.target.value = '';
                      }
                    }}
